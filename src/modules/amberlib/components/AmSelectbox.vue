          ...
          <div class="w-100" :class="{'h-a': getItems.length < 6, 'h-fix-100': getItems.length > 5}">
            <AmScroll :ops="ops">
              <div class="w-100 h-fix-25min py-2 fd-r ai-c px-4 amSelectboxOption fs-5" :title="item.label" :class="[getOptionLine, {active: getActiveMultiple(item.value)}]" v-for="(item, i) in getItems" :key="i" @click="setModelValue(item.value)">
                ...
                {{ item.label }}
                <span class="d-b w-100 h-100 p-a l-0 t-0" :class="[modelData.colorOptionHover]" />
              </div>
            </AmScroll>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    getValue () {
      if (this.modelData.multiple) {
        return Array.isArray(this.modelValue) ? this.modelValue.length > 0 ? `Выбрано ${this.modelValue.length}` : this.modelData.placeholder : this.modelData.placeholder
      } else {
        return this.modelData.items ? this.modelData.items.filter(item => item.value === this.modelValue)[0] ? this.modelData.items.filter(item => item.value === this.modelValue)[0].label : this.modelData.placeholder : this.modelData.placeholder
      }
    },
    getLabelPosition () {
    ...
    getColorSvg () {
      return this.focusValue ? `svg-f-${this.modelData.colorFocus}` : `svg-f-${this.modelData.colorTitle}`
    },
    getOptionsClass () {
      return [1, 2].indexOf(this.getActiveTheme) > -1 ? this.modelData.bottom ? 'b-0' : 't-a' : this.modelData.bottom ? 'b-0' : 't-0'
    },
    getOptionLine () {
    ...
    getItems () {
      return this.modelData.items.filter(item => {
        if (this.searchModel && (this.searchModel.length > 0)) {
          if (item.label.toString().toLowerCase().indexOf(this.searchModel.toLowerCase()) > -1) {
            return item
          }
        } else {
          return item
  ...
  methods: {
    getActiveMultiple (value) {
      return Array.isArray(this.modelValue) ? this.modelValue.indexOf(value) > -1 || false : this.modelValue === value || false
    },
    setModelValue (value) {
      if (this.modelData.multiple) {
    ...
    if (this.modelData.multiple || this.modelData.search) {
      window.addEventListener('click', (e) => {
        if (e.target.closest('.amSelectbox')) {
          if (!e.target.closest('.amSelectbox').isEqualNode(this.$refs.amSelectbox)) {
            this.focusValue = false
          }
        } else {
          this.focusValue = false
...
.amSelectbox {
  ...
  .amSelectboxBlock{
    ...
    input.focus ~ .amSelectboxLine, input:focus ~ .amSelectboxLine {
      left: 0;
      width: 100%;
      opacity: 1;
    }
